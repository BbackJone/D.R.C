...
public class Weapon : MonoBehaviour
{
    ...
    public string m_BulletSort { get; set; }
    public float m_Recoil { get; set; }
    public float m_Weight { get; set; }
    public float m_ShotRate { get; set; }
    public bool m_Autoshot { get; set; }
    public string m_AniTrigger { get; set; }
    ...
    public void Shoot()
    {
        if (m_AmmoBulletNum <= 0)
            return;
        Vector3 RayStartPos = m_Camera.ViewportToWorldPoint(new Vector3(0.5f, 0.5f, 0f));
        RaycastHit hit;
        if (Physics.Raycast(RayStartPos, m_Camera.transform.forward, out hit, 100f))
        {
